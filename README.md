...
Docker Compose does not ship a statically linked binary, however they do ship a Docker container and a runner script 
which accomplishes the same task (see [Alternative install options -> Install as Container](https://docs.docker.com/compose/install/)). There is the added complication that the docker-compose container is not aware of the
entire filesystem, so we have to follow the advice given in the script.
